...
def get_bias_constrained_MLP(feature_number, regularization=5e-5, learning_rate=1e-4, momentum=0.9, batch_size=100, num_epochs = 10, decay_rate=0.9, mode_fast=None):
    """
    ...
    model = Sequential()
    model.add(
        Dense(units=1024, input_dim=feature_number, activation="relu", kernel_regularizer=regularizers.l2(regularization),
              bias_regularizer=regularizers.l2(regularization), bias_constraint=cheb_conv.NonPos()))

    # model.add(Dropout(0.2))

    model.add(Dense(units=1024, activation="relu", kernel_regularizer=regularizers.l2(regularization),
                    bias_regularizer=regularizers.l2(regularization), bias_constraint=cheb_conv.NonPos()))
    ...
    # model.add(Dense(units=32, activation="relu", kernel_regularizer=regularizers.l2(regularization),
    ...
    model.add(Dense(units=5, activation='softmax', kernel_regularizer=regularizers.l2(regularization),
                    bias_regularizer=regularizers.l2(regularization), bias_constraint=cheb_conv.NonPos()))
    # model.add(Dense(units=1, activation='sigmoid', kernel_regularizer=regularizers.l2(regularization),
    #                 bias_regularizer=regularizers.l2(regularization)))
    # model.compile(loss=losses.binary_crossentropy,
    #               # optimizer=optimizers.SGD(lr=learning_rate, momentum=momentum, decay=decay, nesterov=False),
    #               optimizer=optimizers.Adam(lr=learning_rate, decay=decay),
...
def get_cheb_net_model(L, F, K, p, M,
                 ...
                 eval_frequency=200, momentum=0.9, dir_name='', mode_fast=True):
    ...
    for i in range(len(p)):
        if i == 0:
            if mode_fast:
                ...
                g_model.add(cheb_conv.ChebConvSlow(channels=F[i], L=L_param[i], K=K[i], activation='relu', use_bias=True,
                                               input_shape=(feature_num, 1)))
    ...
    for i in range(len(p)):
        if i == 0:
            if mode_fast:
                ...
                g_model.add(
                    cheb_conv.ChebConvSlow(channels=F[i], L=L_param[i], K=K[i], activation='relu', use_bias=True,
                                           input_shape=(feature_num, 1), bias_constraint=cheb_conv.NonPos()))
            print("\n\tCheb_conv, first layer, input shape :", L_param[i].shape[0])
